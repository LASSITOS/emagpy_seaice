...
def fMaxwellQI(cond, depths, s, cpos, hx=0, f=30000, maxiter=50):
    """ Compute the quadrature ratio using Maxwell equation. Same as 'getQs'.
    """
    if len(cond)-1 != len(depths):
        raise ValueError('len(cond)-1 should be equal to len(depths).')
    if len(s) != len(cpos):
...
#    eca0 = fCS(cond, depths, s, cpos, hx=hx) # to kick start the solver
    h = np.r_[depths[0], np.diff(depths)]
    response = np.zeros(len(s),dtype=complex)*np.nan
    cond = cond*1e-3 # convert to S/m
    for i in range(len(s)):
        if hx[i] != 0: # adding air layer of 0 mS/m
            thick = np.r_[hx[i], h]
            ...
            thick = h
            sig = cond
        response[i] = getQ2(cpos[i], s[i], sig, f[i], thick)
    return response
